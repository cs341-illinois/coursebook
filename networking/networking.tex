...
If we need to refer to \keyword{1024} bytes, we will use the more accurate term Kibibyte. Mebibyte and Gibibyte are the analogs of Megabyte and Gigabyte respectively.
...
Therefore we need to manually determine the struct size based on the family.
...
\item Retransmission
  ...
  At the same time, the retransmission shouldn't cause more packets to be dropped.
...
\item Congestion control.
  ...
  Congestion control is to prevent a sender from flooding the network with too many packets.
...
If you want more information, we'll cite the famous article that argued for a particular version \cite{cohen_1980}.
...
The addrinfo structure is passed into \keyword{getaddrinfo} to define the kind of connection you'd like.
...
Sockets are created with a domain \keyword{AF\_INET} for \keyword{IPv4} or \keyword{AF\_INET6} for \keyword{IPv6}, \keyword{socket\_type} is whether to use UDP, TCP, or some other socket type, the \keyword{protocol} is an optional choice of protocol configuration; for our examples we can leave this as 0 for default.
...
There is an old function \keyword{gethostbyname} that is deprecated.
It's the old way to convert a hostname into an IP address.
...
Thus, it is important to check the return value of \keyword{read} and \keyword{write}.
...
\keyword{GET /nosuchfile.html HTTP/1.0} Then the first line includes the well-known \keyword{404} response code.
...
\begin{enumerate}
  ...
  A new socket by itself stores bytes.
  Though we've specified either a packet or stream-based connection, it is unbound to a particular network interface or port.
  ...
  Since a TCP connection is defined by the sender address and port along with a receiver address and port, for a particular server port there can be one passive server socket but multiple active sockets.
...
bind(...);
  ...
  \item \keyword{int listen(int sockfd, int backlog);}
    ...
    These are the connections unassigned to a file descriptor by \keyword{accept}.
...
int client_id = accept(passive_socket,
 ...
 getnameinfo((struct sockaddr *) &clientaddr,
  ...
  Well written code will \keyword{shutdown} a socket before calling \keyword{close} on it.
...
We can assure you that the following code \textit{experiences errors}.
Can you see why?
...
The above code simply sends ``Hello'' through UDP.
...
We may end up trying to passively listen to ourselves, resulting in bind errors.
...
Layer 7 of the OSI model deals with application-level interfaces.
Meaning that you can ignore everything below this layer and treat the Internet as a way of communicating with another computer that can be secure and the session may reconnect.
...
\begin{enumerate}
  \item HTTP(S) - Hypertext Transfer Protocol.
    ...
    The S stands for secure where the TCP connection uses the TLS protocol to ensure that the communication can't be read easily by an onlooker.
...
If the IP address is missing from a machine's cache then it sends a UDP packet to a local DNS server.
...
\item If not, ask higher-level DNS servers for the answer. Cache and send the result
  \item If either packet is not answered within a guessed timeout, resend the request.
\end{enumerate}
...
For example, the Illinois DNS server handles both \keyword{cs.illinois.edu} and \keyword{cs341.cs.illinois.edu}. There is a limit on how many subdomains you can have, but this is often used to route requests to different servers to avoid having to buy many high performant servers to route requests.
...
Read more at \keyword{man 7 epoll} or check out a better version called \keyword{kqueue} in the appendix.
