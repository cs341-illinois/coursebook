...
What this allows is for a process to share the same address space, variables, heap, file descriptors, etc.
...
\item When running an existing and complete program, a new process is required, for example starting `gcc`.
...
Your main function and other functions have automatic variables.
...
The first thread is created when the process starts, the other two after the create.
...
Pthreads let a program set various attributes -- some that resemble the options in clone -- to customize your thread.
...
\item
What does \keyword{pthread\_exit} do if it is not the last thread? What other functions are called after calling pthread\_exit?
\item
