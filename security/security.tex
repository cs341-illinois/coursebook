...
The computer fraud and security act is a broad, and arguably terrible law, that casts any non-authorized use of a `protected computer' as a felony.
...
\item Information Integrity means that only authorized parties are allowed to modify a piece of information, regardless of whether they are allowed to see it.
  It ensures that information remains complete during transit.
...
A lot of the time they are fairly tame, leading to simple program crashes or funny mistakes.
...
If any of the operations would possibly alter the final result, there is a barrier, or if the re-ordering violates the data dependencies of the instructions, the processor keeps the instructions in the stated order \cite[P. 296]{guide2011intel}.
...
What system programmers are worried about is that operations with mutex locks among threads are out of order -- meaning that a pure software implementation of a mutex will fail without copious memory barriers.
...
Since it is still in the cache, if you again trick the processor to read from the cache using \keyword{val} then you will read a memory value that you wouldn't be able to read normally.
...
\item Firewall.
  The Linux kernel provides the netfilter module as a way of deciding whether an incoming connection should be allowed and various other restrictions on connections.
...
\item sudo.
  ...
  Sometimes that would also mean giving users scary system capabilities.
...
\item chroot is a contrived way of creating a virtualization environment.
  ...
  For example with chroot, one can make a hello world program believe \keyword{/home/user/} is actually the root directory.
...
\item Hardware virtualization technology.
  ...
  As such, there can be switches enabled by the user that allow the operating system to flip into a virtualization mode where instructions are run as normal but are monitored for malicious activity.
...
As more and more of our systems are hacked over the web, it is important to understand how we can protect against these attacks.
...
\item Syn-Ack Sequence Number.
  ...
  ISPs help out with the destination problem because they may send a connection through varying routers which makes it hard for an attacker to sit anywhere and be sure that they will receive your packets -- this is why security experts usually advise against using coffee shop wifi for sensitive tasks.
\item Syn-Flood.
...
\item Do buffer overflows / underflows affect the integrity of data?
...
\item Meltdown and Spectre are examples of what kind of security issue? Which one(s) of the triad does it break?
