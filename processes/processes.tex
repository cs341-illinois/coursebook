...
If the stack grows too far -- meaning that it either grows beyond a preset boundary or intersects the heap -- the program will produce a stack overflow error, most likely resulting in a SEGFAULT.
...
One can run out of heap memory if the system is constrained or if a program runs out of addresses, a phenomenon that is more common on a 32-bit system.
...
The reasons for and explanation about this are outside the scope of this class, but it is good to know about its existence.
...
Note, this limit is only for the user, which means if you fork bomb, then you won't be able to kill all created processes since calling \keyword{killall} requires your shell to \keyword{fork()}.
Quite unfortunate.
...
The amazing parallel apparent-O(N) \emph{sleepsort} is today's silly winner.
...
\item If the file descriptor is prepared, it must be inactive in the parent process if the child process is using it or vice versa. A process is using it if it is read or written or if that process \textit{for whatever reason} calls \keyword{exit}. If a process uses it when the other process is as well, the whole application's behavior is undefined.
\end{enumerate}

So how would we fix the code?
We would have to flush the file before forking and refrain from using it until after the \keyword{wait} call -- more on the specifics of this in the next section.
...
Due to event ordering, we need to make sure that the parent process knows that the child is finished using \keyword{wait}.
...
Note that waitpid can be set to be non-blocking, which means it will return immediately, letting a program know if the child has exited.
...
The options are listed below:
  \begin{enumerate}
      \item WNOHANG - Return whether the searched process has exited
...
\begin{enumerate}
\item File descriptors are preserved after an exec. That means if a program opens a file and doesn't close it, it remains open in the child.
...
How would a program alter these in C?
They can call \keyword{getenv} and \keyword{setenv} functions respectively.
...
\item What signal is sent when you press CTRL-C at a terminal?
