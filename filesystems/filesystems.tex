...
This is a historical quirk that was brought about by a clash between network developers and memory/hard storage developers.
...
While its size is sometimes determined by some property of the underlying hardware, it is more frequently determined based on the size of a page of memory for a given system, so that data from the disk can be cached in memory for faster access -- an important feature of many filesystems.
...
This is an important concept, as inodes are stored in the superblock, or some other structure in a well known location with a constant amount of space, indirection allows exponential increases in the amount of space an inode can keep track of.
...
Each pointer refers to a 4KiB disk block, so you can refer to up to $1024*4KiB = 4MiB$ of data.
For the same disk configuration, a double indirect block stores 1024 pointers to 1024 indirection tables.
Thus a double-indirect block can refer to up to $1024 * 4MiB = 4GiB$ of data.
Similarly, a triple indirect block can refer to up to 4TiB of data.
...
Let's think about what directories look like in the actual file system.
...
The other is programs need to explicitly exclude subdirectories from a search, otherwise the search may take a long time.
...
\end{lstlisting}
        ...
        However, symlinks have a key disadvantage, they are slower than regular files and directories.
        ...
        Another disadvantage is that POSIX forbids hard linking directories whereas soft links are allowed.
...
Below is an example that illustrates the simplification of the path \texttt{a/b/../c/.} by using \keyword{cd} in a shell to navigate a filesystem.
...
For each of the three categories, we need to keep track of whether the user is allowed to read the file, write to the file, and execute the file.
...
Write access to a directory will allow a program to create or delete new files or directories inside.
...
Without the execute bit, any attempt to create or remove files or directories will fail since you cannot access them.
...
Every file, upon creation, has an owner, the creator of the file.
...
When a process tries to open a file with a specific mode, its \keyword{uid} and \keyword{gid} are compared with the \keyword{uid} and \keyword{gid} of the file.
...
To simplify our examples, we will be using the command line utility of the same name \keyword{chmod}, short for ``change mode''.
...
It indicates that when run, the program will set the uid of the user to that of the owner of the file.
Similarly, there is a \keyword{setgid} bit which sets the gid of the executor to the gid of the owner.
...
Sticky bits as we use them today serve a different purpose from their initial introduction.
The sticky bit was a bit that could be set on an executable file that would allow a program's text segment to remain in swap even after the end of the program's execution.
...
There is a window of opportunity between when the directory is created and when its permissions are changed.
This leads to several vulnerabilities that are based on a race condition.

If another user replaces \keyword{mystuff} with a hard link to an existing file or directory owned by the second user, then they would be able to read and control the contents of the \keyword{mystuff} directory.
...
\item Some operating systems don't have a true \keyword{/dev/random} like MacOS.
...
\subsection{Updating Modification Time}
        ...
        The \keyword{touch} executable creates a file if it is non-existent and also updates the file's last modified time to be the current time.
...
\item We seek to our desired size and write one byte to ensure that the file is of sufficient length
...
What the file system does, before it completes a potentially expensive operation, is write down what it is going to do in a journal.
In the case of a crash or failure, one can step through the journal and see which files are corrupt and fix them.
This is a way to salvage hard disks in cases where there is critical data and there is no apparent backup.
...
If two disks from opposing raid partitions fail, there is a chance that you can recover though we don't count on it most of the time.
...
In this section, we will go over a fake filesystem and ``walk through'' some examples of how things work.
...
Finding the maximum is slightly more involved.
...
Alternatively always trying to split your data up into chunks that never use indirect blocks may exhaust the limited pool of available inodes.


\subsection{Performing Reads}

Performing reads tends to be pretty easy in our filesystem because our files are compact.
...
Performing writes falls into two categories, writes to files and writes to directories.
First we'll focus on files and assume that we are writing a byte to the $6$th KiB of our file.
To perform a write on a file at a particular offset, first the filesystem must go to the data block that would start at that offset.
...
\begin{itemize}
    \item How would a program perform a write when the last data block is already full?
