...
\item Implement a version of size\_t strlen(const char*) using a loop and no function calls.
...
\item Which one of the following may leave a process running?
...
\item Complete the following code. The following code is supposed to print alternating \keyword{A} and \keyword{B}. It represents two threads that take turns to execute. Add condition variable calls to \keyword{func} so that the waiting thread need not continually check the \keyword{turn} variable. Q: Is pthread\_cond\_broadcast necessary or is pthread\_cond\_signal sufficient?
...
\item A thread unsafe data structure has \keyword{size()} \keyword{enq} and \keyword{deq} methods. Use condition variable and mutex lock to complete the thread-safe, blocking versions.
...
For performance, multiple threads must be able to call \keyword{shortest} at the same time but the graph can only be modified by one thread when no other threads are executing inside \keyword{shortest} or \keyword{set\_edge}.
...
\item What are the Coffman conditions and what do they mean? Can you provide a definition of each one and an example of breaking them using mutexes?
...
\item What is a UID? GID? What is the difference between UID and Effective UID?
\item What is umask?
...
\item Create a TCP client that sends ``Hello'' to a server.
...
\item What are the minimum network calls required to build a TCP server? What is their correct order?

\item What are the minimum network calls required to build a TCP client? What is their correct order?
...
\item Give the name of a signal that can not be caught by a signal handler.
...
\item What attributes are passed over to process children? How about executed processes?
\end{enumerate}
