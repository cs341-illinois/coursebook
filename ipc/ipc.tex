...
We can also number our pages (``Page 0'' ``Page 1'' etc). Let's do a sample calculation of how many pages there are. Assume a page size of 4KiB.
...
A \textbf{frame}, sometimes called a `page frame', is a block of \emph{physical memory} or RAM -- Random Access Memory.
...
On a machine with page size 256 Bytes, the lowest 8 bits (10101010) will be used as the offset.
...
Each table is a list of pointers that point to the next level of tables, some sub-tables may be omitted.
As an example, a two-level page table for a 32-bit architecture is shown below.
...
First, the MMU takes the top-level page table and finds the \keyword{Index1}'th entry.
...
Then, the MMU adds the offset and does the read or write.
...
\begin{enumerate}
	...
	\item \textbf{Invalid} When a program tries to write to a non-writable memory address or read from a non-readable memory address.
...
Page faults are so powerful because they let the operating system take control of when a file is used.
...
This piece of code allocates space for 100 integers and creates a piece of memory that is shared between all processes.
The code then forks.
...
Cut takes a delimiter, in this case, a dot, and a field position, in our case 1, and outputs per line the nth field by each delimiter.
...
If the program already has a file descriptor, it can `wrap' it into a FILE pointer using \keyword{fdopen}.
...
This is useful if you want to have a pipe between two processes without one process having to fork the other process.
...
\begin{itemize}
	...
	\item \keyword{fread} reads a certain number of bytes from a file. An error is returned if already at the end of the file, at which point the program must call \keyword{feof()} to check if the program attempted to read \textit{past} the end of the file.
	...
	\item \keyword{feof} Returns true if you are at the end of a file
 	\item \keyword{ferror} Returns true if an error occurred reading, writing or seeking.
