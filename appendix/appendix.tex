...
\item \keyword{man} every system programmer's favorite command tells you more about all your favorite functions!
...
A pipe takes the \keyword{stdout} of the program on its left and feeds it to the \keyword{stdin} of the program on its right.
...
For example, you can write a C program that loops through all of the time zones and executes the \keyword{date} command to print out the date and time in all locales. Environment variables are used for all sorts of programs so modifying them is important.
...
Other times, a processor supports unaligned access, leaving the compiler able to pack structs.
What does this mean?
...
If a buffer is too small for some data (e.g.~input values from the user), then there is a real possibility that other stack variables and even the return address will be overwritten.
...
\begin{enumerate}
\item Preprocessing: The preprocessor expands all preprocessor directives.
...
A static library is literally an archive of object files.
...
While the API for most filesystems has stayed the same on POSIX over the years, the actual filesystems themselves provide lots of important aspects.
...
\item Efficient Backups. Many of us have data that we can't store on the cloud for one reason or another. It is useful that when a filesystem is either being used as a backup medium or is the source to the backup that it is able to calculate what has changed efficiently, compress files, and sync between the external drive.
\item Integrity and Bootability. File systems need to be resilient to bit flipping. Most readers have their operating system installed on the same partition as the file system that they used to do different operations. The file system needs to make sure a stray read or write doesn't destroy the boot sector -- meaning your computer can't start up again.
...
\item Distributed. Sometimes, the filesystem should be single machine fault tolerant. Hadoop and other distributed file systems allow you to do that.
\end{itemize}
...
A common usage can be imagined as having a fast, but small capacity SSD and a slower, large capacity HDD.
...
By the time the operating system needs them, they are already on the SSD.
StoreMI also does this with other applications as well.
The technology still leaves a lot to be desired, but it is an interesting intersection of data and pattern matching with filesystems.
...
Although it is called the Fair Scheduler there are a fair number of problems.
...
Peterson's algorithm is used to implement low-level Linux Kernel locks for the Tegra mobile processor (a system-on-chip ARM processor and GPU core by Nvidia) \href{https://android.googlesource.com/kernel/tegra.git/+/android-tegra-3.10/arch/arm/mach-tegra/sleep.S\#58}{Link to Lock Source}
...
Thus data changes, such as the state of a flag and turn variable in the above example, may not be shared between two CPU cores.
...
Also, there are CPU instructions to ensure that main memory and the CPU's cache are in a reasonable and coherent state.
Higher-level synchronization primitives, such as \keyword{pthread\_mutex\_lock} will call these CPU instructions as part of their implementation.
...
For further reading, we suggest the following web post that discusses implementing Peterson's algorithm on an x86 processor and the Linux documentation on memory barriers.
\begin{enumerate}
...
\item That thread calling broadcast is stopped before it wakes any threads.
...
\item With our implementation of a condition variable, this would be terminated.
  If you had an implementation that appended to the tail of the list and iterated from the head to the tail, this could go on infinitely many times.
\end{enumerate}
...
This model was introduced so that there can be an Acquire/Release/Consume model without mixing in Relaxed because Consume is similar to relaxed.
...
Let the maximum amount of time that a process runs be equal to $S$.
...
\begin{enumerate}
  ...
  Under a non-strict analysis of processor sharing though, the number of times that the scheduler waits is best approximated by the number of times the scheduler needs to wait.
...
\item If $E[W_i]$ is the wait time for priority $i$,
  ...
  This means that if one is priority 0, then a process only needs to wait for the other P0 processes which there should be $\rho C/ (1 - \rho_0)$ P0 processes arrived before to process in FCFS order.
...
\begin{enumerate}
\item Response Time. Just a heads up, this isn't going to be pretty.
  \[
  ...
  If lower priority jobs come in at a higher service time variance, that means our average response times could go down, unless they make up most of the jobs that come in.
  ...
  The other extreme is if one percent of jobs are high priority and they come in at a low variance.
  That means the chances of the system getting a high priority job that will take a long time is low, thus making our response times lower on average.
...
\begin{enumerate}
  ...
  \item The next two octets specify the total length of the datagram.
    ...
    This is given in multiples of octets, meaning that a value of 20 means 20 octets.
  ...
  \item The next octet is the protocol number.
    Although protocols between different layers of the OSI model are supposed to be black boxes, this is included, so that hardware can peer into the underlying protocol efficiently.
  ...
  \item The next two octets are an internet checksum.
