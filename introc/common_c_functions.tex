...
Usually, standard input is sourced from the terminal in which the program is being run, and standard out is the same terminal.
...
\keyword{printf} is a C library function, while \keyword{write} is a system call.
...
We would use \keyword{sprintf} if the size of the printed string is less than the provided buffer -- think about printing an integer, it will never be more than 11 characters with the NUL byte.
...
\keyword{gets} is deprecated in the C99 standard and has been removed from the latest C standard (C11).
...
On the other hand, one of the advantages of \keyword{getline} is that it will automatically allocate and reallocate a buffer on the heap of sufficient size.
...
Now, scanf will keep reading characters until the string ends.
...
One last thing to note is that if system calls are expensive, the \keyword{scanf} family is much more expensive due to compatibility reasons.
Since it needs to be able to process all of the printf specifiers correctly, the code isn't efficient \todo{citation needed}.
...
\begin{itemize}
    ...
    \textbf{This function assumes that there is enough space for \keyword{src} at the end of destination including the NUL byte}
...
\end{lstlisting}
	...
	\item For integer parsing use
	      ...
	      hexadecimal etc) and an optional pointer \keyword{endptr} and return a
	      parsed value.
