...
\end{lstlisting}
        ...
        Continue will skip the rest of the loop body and set the program counter back to the start of the loop.
...
\end{lstlisting}
        ...
        The reason is that it makes your code infinitely harder to understand when strung together with multiple chains, which is called spaghetti code.
...
\end{lstlisting}
        ...
        If the function is \keyword{void} then it simply exits the function.
...
\end{lstlisting}
	    \item \keyword{unsigned} is a type modifier that forces \keyword{unsigned} behavior in the variables it modifies.
...
\end{lstlisting}
    ...
    If the first operand is not zero, then the second won't be evaluated and the expression will evaluate to 1.
