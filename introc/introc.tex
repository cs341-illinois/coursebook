...
Note: This chapter is long and goes into a lot of detail. Feel free to gloss over parts with which you have experience.

C is the de-facto programming language to do serious system programming.
Why?
Most kernels have their API accessible through C.
The Linux kernel \cite{Love} and the XNU kernel \cite{xnukernel} on which MacOS is based are written in C and have a C API - Application Programming Interface.
The Windows Kernel uses C++, but doing system programming on that is much harder on Windows than UNIX for novice system programmers.
