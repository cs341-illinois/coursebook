...
We will often use the following typedef, so we can use the struct name as the full type.
...
The phone variable starts at the \keyword{0x128}th byte and continues for sizeof(int) bytes with this compiler.
...
\caption{Struct pointing to 11 boxes, 4 filled with 0006, 7 the string ``person''}
\end{figure}
...
Here are some common ways to initialize a string. Where do they reside in memory?
